...
final class HuxDiscoveryUnitTest extends UnitTestCase {
  ...
  public function testHuxDiscovery(): void {
    ...
    $discovery = new HuxDiscovery(['foo.bar.service' => ['foo', HuxTestHooks::class]]);
    $discovery->discovery($containerBuilder);

    $this->assertCount(1, $discovery->getHooks('test_hook'));
    $this->assertCount(1, $discovery->getHooks('test_hook_returns'));
    $this->assertCount(0, $discovery->getHooks('test_hook_doesnt_exist'));

    /** @var \Drupal\hux\HuxDiscovery $discovery */
    $discovery = \unserialize(\serialize($discovery));
    $this->expectException(\Exception::class);
    $this->expectDeprecationMessage('Hook implementations were cleared after serialization. Re-construct the discovery class.');
    $discovery->discovery($containerBuilder);
  }
